...
-- NOTE:
-- The WITH clause is considered “temporary” because the result is not permanently stored anywhere
-- in the database schema. It acts as a temporary view that only exists for the duration of the query.
-- This veiw is available i the scope of the connection/transaction session scope, on the transaction is commited
-- or the connection is closed the session goes away. If you have some kind of query rerun again and again to
-- compute some kind of aggregation which is used by the subsequent queries, it is best toi create this table using
-- the "WITH clause", this way a view for this query is created and will be reused without
-- the query getting reexecuted - think this view is cached for the txn/connection scope
...
--
-- Windows Function
-- Observe unlike regular aggregate functions, use of a window function does not cause rows to become
-- "grouped into a single output row" — the rows retain their separate identities.
--
-- Added the fourth table "Runners" table.
-- Consider a query where we wanted to find the average weight of the runners grouped
-- by country. This can be done like this:
--
select
country,
avg(weight)
...
-- observe "over ()" in the following, starting to use Windows Function
